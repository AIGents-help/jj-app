...
/* ════════════════════════════════════════════════════════════════════
   MonthReveal — the Day-31 mechanic for Reveal — The Awareness Journal.
...
export default function MonthReveal({dayN=1,monthMap={},outcomes,onClose,onComplete}){
  ...
  async function finish(){
    ...
    await sset(`journey:archive:month:${prevNum}`,{monthMap,outcomes,stats:{logged,complete,rate,avgScore,avgRate,avgEffort},ts:Date.now()});
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:60,overflowY:"auto"}}
      ...
      <div style={{background:PAPER,maxWidth:500,margin:"20px auto",borderRadius:4,overflow:"hidden"}}>
        ...
        <div style={{background:NAVY,padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{color:"#8FA0BC",fontSize:9.5,letterSpacing:".2em"}}>MONTH {monthNum} · DAY {dayN}/31</div>
        ...
        {step==="reveal"&&(
          <div style={{padding:20}}>
            ...
            <div style={{fontFamily:"Georgia,serif",fontSize:13.5,color:NAVY2,marginTop:16,lineHeight:1.6}}>
              The patterns were always there. The month didn't deliver them — it revealed them.
            </div>
        ...
        {step==="reckoning"&&(
          <div style={{padding:20}}>
            ...
            <div style={{fontSize:11.5,color:GRAY,marginBottom:16}}>
              Each outcome is a decision — on purpose. Nothing rolls into next month in the dark.
            </div>
            ...
            {liveOutcomes.length===0?(
              <div style={{border:`1px dashed ${LINE}`,borderRadius:3,padding:"18px 16px",textAlign:"center",color:GRAY,fontSize:13}}>
                No outcomes were set this month. Nothing to carry — the next month starts clean.
              </div>
            ):liveOutcomes.map(o=>{
