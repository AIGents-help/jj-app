import { activationTrend, currentStreak } from "./activation.js";

// revealSignals.js
// Deterministic awareness-detection layer for Reveal — The Awareness Journal.
...
// Loads raw (unadapted) entries for activation functions that read via FIELDS.
function loadRawEntries(windowDays, today) {
  if (typeof localStorage === "undefined") return [];
  const out = [];
  for (let i = windowDays - 1; i >= 0; i--) {
    const d = new Date(today);
    d.setDate(d.getDate() - i);
    const raw = localStorage.getItem(KEY_PREFIX + ymd(d));
    let entry = null;
    if (raw) try { entry = JSON.parse(raw); } catch { /* skip */ }
    out.push({ entry, isFuture: false });
  }
  return out;
}

export function buildPatternSignals(opts = {}) {
  const { windowDays = 31, today = new Date() } = opts;
  const { signals } = computeSignals(opts);
  const base = signals.filter((s) => s.kind === "activation-break" || s.kind === "alignment-break");

  const chronoDays = loadRawEntries(windowDays, today);
  const trend = activationTrend(chronoDays);
  const streak = currentStreak(chronoDays);
  const activationSignals = [];

  if (trend.delta != null && trend.delta < -0.1) {
    activationSignals.push({
      kind: "activation_drift",
      facts: { recent: trend.recent, prior: trend.prior, delta: trend.delta },
      coachLine: `Completion rate is down ${Math.round(Math.abs(trend.delta) * 100)}% on the week. You're aware, aligned — but Activation is leaking. The loop breaks at the last A. What got set and walked past?`,
    });
  } else if (trend.delta != null && trend.delta > 0.1) {
    activationSignals.push({
      kind: "activation_climb",
      facts: { recent: trend.recent, prior: trend.prior, delta: trend.delta },
      coachLine: `Up ${Math.round(trend.delta * 100)}% on Activation this week. That's not motivation — that's a decision compounding. Hold the standard.`,
    });
  }

  if (streak >= 3) {
    activationSignals.push({
      kind: "activation_streak",
      facts: { streak },
      coachLine: `${streak} days straight, full Top 3 closed. Fractal conduct — the small proof creates the large evidence. Don't let one easy 'later' quietly end it.`,
    });
  }

  return [...base, ...activationSignals];
}
